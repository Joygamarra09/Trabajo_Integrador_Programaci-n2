...
public class MenuHandler {
    ...
    public void crearUsuario() {
        ...
        try {
            ...
            Usuario nuevoUsuario = new Usuario(nombre, apellido, username, email, true, LocalDateTime.now(), 0L, false);
            CredencialAcceso nuevaCredencial = new CredencialAcceso(password, "salt_de_prueba", LocalDateTime.now(), false, 0L, 0L, false);
    ...
    public void actualizarUsuario() {
        ...
        Long id = leerIdSeguro(); // Usamos un helper para leer el ID
        if (id == null) return; // <-- Cambia la comprobación a 'null'  // El helper ya mostró el error
    ...
    /**
     * Lee un ID (Long) del scanner y se asegura de que sea un número válido.
     * @return El ID como Long (objeto), o null si la entrada fue inválida.
     */
    private Long leerIdSeguro() {
        try {
            String input = scanner.nextLine();
            // 1. Cambiamos Integer.parseInt por Long.parseLong
            return Long.parseLong(input); 
        } catch (NumberFormatException e) {
            display.mostrarError("Entrada inválida. Se esperaba un número.");
            // 2. Devolvemos null en lugar de -1
            return null; 
        }
    }
}
